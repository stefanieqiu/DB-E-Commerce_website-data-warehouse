...
<html>
  <head>
	...
	function searchStore(){
		...
		th.action="<%=path%>/servlet/StoreAction?action_flag=search";
		th.submit();
	}
	
	function first(){
		
		window.location.href = "<%=path%>/servlet/StoreAction?action_flag=search&pageNum=1";
		
	}
	function next(){
		
		window.location.href = "<%=path%>/servlet/StoreAction?action_flag=search&pageNum=<%=dividePage.getCurrentPage()+1%>";		
	...
	function end(){
		...
		window.location.href = "<%=path%>/servlet/StoreAction?action_flag=search&pageNum=<%=dividePage.getPageCount() %>";
			
	}
	
	function changePage(currentPage){
	
		window.location.href = "<%=path%>/servlet/StoreAction?action_flag=search&pageNum="+currentPage;
	...
	function del(){
		...
		th.action="<%=path%>/servlet/StoreAction?action_flag=del";
		th.submit();		
	...
	function view(){
		...
		var th = document.form1;
		//var th = ids;
                th.action="<%=path%>/servlet/StoreAction?action_flag=view&oid="+getSelectedValue();
		th.submit();		
   	...
   	<tr>
   		...
   		<table border=1 width=100%>
   			<tr align="center">
   				...
   				<td width=15%>sid</td>
   				<td width=15%>zip</td>
                                <td width=15%>salesman number</td>
   			...
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map.get("sid") %>"/></td>
   				<td width=15%><%=map.get("sid") %></td>
   				<td width=15%><%=map.get("zip") %></td>
                                <td width=15%><%=map.get("num") %></td>
                                <td width=15%><%=map.get("rname") %></td>
        ...
        <tr>
                <td colspan="2" align="center">
   					<!--<button type="button" onclick="searchProduct()" >search</button>-->
                                        <br>
                                        <br>
   					<button type="button" style="width: 65px; height: 25px;"onclick="javascript:location.href='<%=path %>/addStore.jsp'">add</button>
                                        <!--<span width = 30%></span>-->
